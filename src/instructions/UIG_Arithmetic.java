...
public abstract class UIG_Arithmetic extends AbstractInstruction {
	...
	public final boolean check(ErrorHandler hErr, Module module) {
		...
		if(this.hasOperand("FL") && this.hasOperand("DM") || this.hasOperand("EX") && this.hasOperand("DM")) {
			Operand o = getOperandData("DM");
			Value constantSize = module.evaluate(o.expression, true, BitLocation.LowAddress ,hErr, this,
			...
			if(this.hasOperand("DM")){
				...
				//error missing dest
				isValid = false;
				hErr.reportError(makeError("instructionMissingOp", this.getOpId(), "{DM,DX}, {DR}, or {DX}"), this.lineNum, -1);
			}
			...
			if(this.hasOperand("FM")){
				...
				isValid = false;
				hErr.reportError(makeError("instructionMissingOp", this.getOpId(), "{FM,FX}, {FR}, or {FX}"), this.lineNum, -1);
			}
